...
export default function Home() {
  ...
  const [verifyEmailErr, setVerifyEmailErr] = useState<string | null>(null);
  const [verifyPhoneErr, setVerifyPhoneErr] = useState<string | null>(null);
  const [verifyEmailLoading, setVerifyEmailLoading] = useState(false);
  const [verifyPhoneLoading, setVerifyPhoneLoading] = useState(false);
  const verifyTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // ── Check if user needs verification (5s delay after arriving on Home) ──
  // Re-triggers each time the screen is focused if not yet verified.
  useFocusEffect(
    useCallback(() => {
      if (!user) return;
      const emailOk = user.email_verified;
      const phoneOk = user.phone_verified;
      if (emailOk && phoneOk) return; // already verified, skip
  ...
  return (
    <SafeAreaView edges={["top"]} style={{ flex: 1, backgroundColor: colors.neutrals.background }}>
      ...
      {/* ── Vérification email/phone séquentielle ── */}
      <VerificationModal
      ...
      >
        ...
        <View style={{ marginTop: spacing.lg }}>
          <FlooMoneyCard balance={wallet?.balance ?? 0} currency={getLocalCurrency(user.country, wallet?.currency)} fullName={user.full_name} profileId={user.profile_id} compact />
        </View>
