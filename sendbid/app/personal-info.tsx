...
import { View, StyleSheet, Alert, TouchableOpacity, Platform, Modal, ScrollView as SV } from "react-native";
...
export default function PersonalInfo() {
  ...
  return (
    <Screen title="Mes informations personnelles" back hero>
      ...
      {/* Pays / Devise */}
      <TouchableOpacity onPress={() => setShowCountryPicker(true)} style={styles.countryBtn} activeOpacity={0.7}>
        <Ionicons name="flag-outline" size={18} color={colors.neutrals.textSecondary} />
        <View style={{ flex: 1, marginLeft: 10 }}>
          <TText variant="label" color={colors.neutrals.textSecondary}>Pays (devise locale)</TText>
          <TText variant="body" weight="semiBold" style={{ marginTop: 2 }}>{country || "Sélectionner un pays…"}</TText>
        </View>
        {country ? (
          <View style={styles.currencyBadge}>
            <TText variant="label" weight="extraBold" color={colors.primary.base}>{COUNTRY_CURRENCY[country]}</TText>
          </View>
        ) : null}
        <Ionicons name="chevron-down" size={16} color={colors.neutrals.textTertiary} style={{ marginLeft: 6 }} />
      </TouchableOpacity>

      {/* Modal sélecteur de pays */}
      <Modal visible={showCountryPicker} transparent animationType="slide" onRequestClose={() => setShowCountryPicker(false)}>
        <TouchableOpacity activeOpacity={1} style={styles.overlay} onPress={() => setShowCountryPicker(false)}>
          <TouchableOpacity activeOpacity={1} style={[styles.sheet, { maxHeight: "70%" }]}>
            <TText variant="subtitle" weight="extraBold" style={{ marginBottom: 12 }}>Choisir un pays</TText>
            <SV showsVerticalScrollIndicator={false}>
              {COUNTRIES.map((c) => (
                <TouchableOpacity
                  key={c}
                  onPress={() => { setCountry(c); setShowCountryPicker(false); }}
                  style={[styles.countryRow, country === c && { backgroundColor: colors.overlays.primarySoft }]}
                >
                  <TText variant="body" weight={country === c ? "bold" : "regular"}>{c}</TText>
                  <TText variant="caption" color={colors.primary.base} weight="extraBold">{COUNTRY_CURRENCY[c]}</TText>
                </TouchableOpacity>
              ))}
            </SV>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  countryBtn: { flexDirection: "row", alignItems: "center", backgroundColor: colors.neutrals.surface, borderWidth: 1.5, borderColor: colors.neutrals.border, borderRadius: radii.lg, padding: spacing.md, marginBottom: spacing.md },
  currencyBadge: { backgroundColor: colors.overlays.primarySoft, paddingHorizontal: 10, paddingVertical: 4, borderRadius: radii.full },
  countryRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 12, paddingHorizontal: spacing.sm, borderRadius: radii.md, marginBottom: 2 },
});
