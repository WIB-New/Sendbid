      ...
      <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
        <View style={styles.topBar}>
          ...
          <TouchableOpacity testID="onboarding-skip" onPress={() => router.replace("/welcome")}>
            <TText variant="caption" weight="semiBold" color="#6B7280">
              Ignorer
            </TText>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.content}>
          {/* Illustrated circular hero with floating badges */}
          ...
          <TText variant="title" weight="extraBold" color="#1A2B4C" align="center" style={styles.title}>
            {cur.title}
          </TText>
          <TText variant="body" color="#6B7280" align="center" style={styles.desc}>
            {cur.desc}
          </TText>
        </View>
        ...
        <View style={[styles.cta, { paddingBottom: Math.max(spacing.xl, insets.bottom + spacing.lg) }]}>
          ...
          <Pressable onPress={() => setStep((s) => Math.max(0, s - 1))} style={styles.backRow} disabled={step === 0}>
            <TText variant="caption" color={step === 0 ? "#D1D5DB" : "#1A2B4C"} weight="semiBold" align="center">
              {step > 0 ? "Précédent" : " "}
            </TText>
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  dot: { height: 8, borderRadius: radii.full, backgroundColor: "#E5E7EB" },
  ...
  ringOuter: { width: 210, height: 210, borderColor: "rgba(0,0,0,0.06)" },
  ringMid: { width: 170, height: 170, borderColor: "rgba(0,0,0,0.03)" },
  badge: {
    position: "absolute",
    ...
    borderWidth: 1.5,
    shadowColor: "#1A2B4C", shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 4 },
    elevation: 4,
  },
  ...
  finalBtn: { backgroundColor: "#1A2B4C" },
