...
import { View, StyleSheet, Platform, Alert, TouchableOpacity, ActivityIndicator } from "react-native";
...
export default function Limits() {
  ...
  useEffect(() => {
    api.get("/limits/upgrade-request/status")
      .then((res: any) => setHasPendingRequest(res.has_pending))
      .catch(() => {})
  ...
  return (
    <Screen title="Limites de transfert" back hero>
      ...
      {loading ? (
        <View style={[styles.upgradeCard, { backgroundColor: colors.neutrals.border, justifyContent: "center" }]}>
          <ActivityIndicator color={colors.primary.base} />
        </View>
      ) : hasPendingRequest ? (
        <View style={[styles.upgradeCard, { backgroundColor: colors.neutrals.border }]}>
          <View style={styles.upgradeIcon}>
            <Ionicons name="time-outline" size={26} color={colors.neutrals.textSecondary} />
          </View>
          <View style={{ flex: 1, marginLeft: 14 }}>
            <TText variant="body" weight="extraBold" color={colors.neutrals.textSecondary}>Demande en cours</TText>
            <TText variant="caption" color={colors.neutrals.textSecondary} style={{ marginTop: 4, lineHeight: 18 }}>
              Votre demande d'augmentation est en cours de traitement. Réponse sous 48h.
            </TText>
          </View>
        </View>
      ) : (
