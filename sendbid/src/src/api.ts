...
if (!BASE) {
  console.warn("[API] EXPO_PUBLIC_BACKEND_URL n'est pas définie. Vérifiez votre fichier .env");
}
...
export function apiError(e: any): string {
  ...
  if (status === 401) return "Session invalide ou code incorrect. Veuillez vous reconnecter.";
