...
const TYPE_CONFIG: Record<ToastType, {
  ...
  info: { icon: "information-circle", color: colors.primary.base, bg: colors.overlays.infoSoft },
};
    ...
    >
      <View style={[styles.toast, {
        ...
        <View style={styles.content}>
          <TText variant="caption" weight="extraBold" color={colors.neutrals.textPrimary} numberOfLines={1}>
            {toast.title}
          </TText>
          {toast.message ? (
            <TText variant="label" color={colors.neutrals.textSecondary} numberOfLines={3} style={{ marginTop: 2 }}>
              {toast.message}
            </TText>
          ) : null}
        </View>
        ...
        >
          <Ionicons name="close" size={isSmallScreen ? 14 : 16} color={colors.neutrals.textTertiary} />
        </TouchableOpacity>
      </View>
    </Animated.View>
  );
