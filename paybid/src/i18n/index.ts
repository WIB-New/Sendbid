...
export const i18n = new I18n({ fr, en, es, ar, de, it, pt, zh });
 ...
 */
export const useLocale = create<{ locale: LocaleCode; tick: number; setLocale: (c: LocaleCode) => Promise<void> }>((set) => ({
  locale: "fr",
  tick: 0,
  setLocale: async (code) => {
    ...
    set(s => ({ locale: code, tick: s.tick + 1 }));
  },
}));
...
/** Hook de traduction — utilise la locale active du store */
export function useTranslation() {
  const locale = useLocale((s) => s.locale);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  void locale; // force re-render quand la langue change
  return { t: (key: string, options?: object) => i18n.t(key, options) };
}
