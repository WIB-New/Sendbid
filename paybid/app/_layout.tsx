...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    if (appVariant === "paybid") {
      ...
      if (!user) {
        if (!inPaybid || (segs[1] !== "login" && segs[1] !== "signup" && segs[1] !== "create-pin" && segs[1] !== "onboarding")) {
          // Vérifier si onboarding déjà vu
          AsyncStorage.getItem("paybid_onboarded").then((val: string | null) => {
            if (!val) router.replace("/paybid/onboarding" as any);
            else router.replace("/paybid/login" as any);
          }).catch(() => router.replace("/paybid/login" as any));
        }
        return;
      }
      if (!isAgent) {
