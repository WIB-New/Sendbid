...
export default function PaybidLogin() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Si l'agent n'a pas encore de PIN, le rediriger vers la création
      if (!me.data.user.has_pin && !me.data.user.pin_hash) {
    ...
    <View style={{ flex: 1 }}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.formWrap}>
        <SafeAreaView edges={["bottom"]} style={styles.form}>
          ...
          />
          <Input testID="paybid-password" label="Mot de passe" value={password} onChangeText={setPassword} icon="lock-closed-outline" passwordToggle secureTextEntry
            labelColor={paybidColors.neutrals.textSecondary}
            boxStyle={{ backgroundColor: paybidColors.neutrals.surface, borderColor: paybidColors.neutrals.border }}
            placeholderTextColor={paybidColors.neutrals.textTertiary}
            style={{ color: paybidColors.neutrals.textPrimary }}
...
const styles = StyleSheet.create({
  ...
  formWrap: { flex: 1.5, backgroundColor: paybidColors.neutrals.background },
