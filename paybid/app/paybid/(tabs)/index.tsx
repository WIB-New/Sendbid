...
import { View, StyleSheet, ScrollView, RefreshControl, TouchableOpacity, Animated, Easing, Modal, Image as RNImage, ActivityIndicator } from "react-native";
    ...
    <SafeAreaView edges={["top"]} style={{ flex: 1, backgroundColor: paybidColors.neutrals.background }}>
      ...
      >
        ...
        <View style={styles.header}>
          <TouchableOpacity style={styles.avatar} onPress={() => router.push("/paybid/(tabs)/profile")}>
            {agent.avatar_url ? <RNImage source={{ uri: agent.avatar_url }} style={styles.avatarImg} /> : <Ionicons name="person" size={20} color={paybidColors.primary.base} />}
          </TouchableOpacity>
          ...
          <TouchableOpacity onPress={() => router.push("/paybid/notifications" as any)} style={styles.notifBtn}>
            <Ionicons name="notifications-outline" size={22} color={paybidColors.neutrals.textPrimary} />
            {unread > 0 && (
              <View style={styles.badge}>
                <TText style={{ color: "white", fontSize: 9, fontWeight: "bold" }}>{unread > 9 ? "9+" : String(unread)}</TText>
              </View>
            )}
          </TouchableOpacity>
        </View>
        ...
        <View style={[styles.heroCard, shadows.lg, { backgroundColor: "#54280f" }]}>
          ...
          <View style={styles.balanceLabelRow}>
            ...
            <TouchableOpacity onPress={handleEyePress} style={styles.eyeBtn}>
      ...
      <Modal visible={pinModal} transparent animationType="fade" onRequestClose={() => setPinModal(false)}>
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "flex-end" }}>
          <View style={{ backgroundColor: paybidColors.neutrals.surface, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: spacing.xl, paddingBottom: 40 }}>
            <View style={{ alignItems: "center", marginBottom: spacing.lg }}>
              <View style={{ width: 48, height: 4, borderRadius: 2, backgroundColor: paybidColors.neutrals.border, marginBottom: spacing.lg }} />
              <Ionicons name="lock-closed" size={32} color={paybidColors.primary.base} />
              <TText variant="subtitle" weight="bold" style={{ marginTop: 8 }}>Saisir votre PIN</TText>
              <TText variant="caption" color={paybidColors.neutrals.textSecondary} style={{ marginTop: 4 }}>Pour voir votre solde</TText>
            </View>
            {/* Points PIN */}
            <View style={{ flexDirection: "row", justifyContent: "center", gap: 12, marginBottom: spacing.lg }}>
              {[0,1,2,3,4,5].map(i => (
                <View key={i} style={{ width: 13, height: 13, borderRadius: 7, backgroundColor: i < pinInput.length ? paybidColors.primary.base : paybidColors.neutrals.border }} />
              ))}
            </View>
            {pinError && <TText variant="caption" color="#EF4444" align="center" style={{ marginBottom: 8 }}>PIN incorrect. Réessayez.</TText>}
            {/* Clavier ou spinner */}
            {pinLoading ? (
              <ActivityIndicator color={paybidColors.primary.base} size="large" style={{ marginVertical: 24 }} />
            ) : (
              <View style={{ flexDirection: "row", flexWrap: "wrap", justifyContent: "center" }}>
                {["1","2","3","4","5","6","7","8","9","","0","⌫"].map((k, i) => (
                  <TouchableOpacity key={i} style={{ width: "33%" as any, height: 60, justifyContent: "center", alignItems: "center" }}
                    onPress={() => k === "⌫" ? setPinInput(p => p.slice(0,-1)) : k ? handlePinDigit(k) : null}
            ...
            <TouchableOpacity onPress={() => setPinModal(false)} style={{ marginTop: 8, alignItems: "center" }}>
...
const styles = StyleSheet.create({
  ...
  notifBtn: { width: 40, height: 40, borderRadius: radii.full, backgroundColor: paybidColors.neutrals.surface, borderWidth: 1, borderColor: paybidColors.neutrals.border, alignItems: "center", justifyContent: "center", marginLeft: 8 },
  badge: { position: "absolute", top: -4, right: -4, minWidth: 18, height: 18, paddingHorizontal: 4, borderRadius: 9, backgroundColor: "#EF4444", alignItems: "center", justifyContent: "center", zIndex: 10 },
